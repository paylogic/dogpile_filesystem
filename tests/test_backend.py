    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.skip(
                reason="Recursive usage with dogpile thread lock is broken"
            ),
        ),
        pytest.param(
            None,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            lambda _: tempfile.NamedTemporaryFile(delete=False),
            id="tempfile.NamedTemporaryFile",
        ),
        pytest.param(
            lambda _: tempfile.NamedTemporaryFile(delete=True),
            id="tempfile.NamedTemporaryFile(delete)",
        ),
        ...
        pytest.param(
            lambda tmpdir: os.fdopen(
                os.open(str(tmpdir / "asd"), os.O_RDWR | os.O_CREAT), "w+b"
            ),
            id="os.fdopen",
        ),
    ],
...
def test_cleanup_does_not_get_stuck_in_case_files_are_not_deletable(region):
    ...
    with mock.patch.object(
        backend.RawFSBackend, "prune", return_value=None, autospec=True
    ) as m:
        assert fn("foo", 10000)
        assert fn("bar", 10000)
        assert fn("baz", 10000)
